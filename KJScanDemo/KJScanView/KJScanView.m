...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        _animationImageSize = CGSizeMake(self.scanRetangleRect.size.width, 4);
        _animationImageTime = 2.0;
...
- (void)drawRect:(CGRect)rect{
    ...
    UILabel *descLabel  = [[UILabel alloc] initWithFrame:CGRectMake(0, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 16, rect.size.width, 20)];
    descLabel.font = [UIFont systemFontOfSize:15];
    descLabel.textAlignment = NSTextAlignmentCenter;
    descLabel.textColor = UIColor.whiteColor;
    descLabel.text = @"将二维码置于框内进行扫描";
    [self addSubview:descLabel];
    
//    UIButton *myCodeButton = [[UIButton alloc] initWithFrame:CGRectMake(0, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 10 + 20 + 10, rect.size.width, 20)];
//    self.myCodeButton = myCodeButton;
//    myCodeButton.titleLabel.font = [UIFont systemFontOfSize:15];
//    [myCodeButton setTitle:@"我的二维码" forState:UIControlStateNormal];
//    [myCodeButton setTitleColor:_colorAngle forState:UIControlStateNormal];
//    [myCodeButton addTarget:self action:@selector(myCodeClickAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)myCodeClickAction:(UIButton*)sender{
    if(self.myQRCodeBlock){
        self.myQRCodeBlock();
    }
}
/// 开始扫描
- (void)startScan{
    if (_isAnimating == NO) return;
    
    __weak __typeof(self) weakSelf = self;
    [UIView animateWithDuration:self.animationImageTime animations:^{
        weakSelf.scanLine.frame = CGRectMake(weakSelf.scanRetangleRect.origin.x, weakSelf.scanRetangleRect.origin.y + weakSelf.scanRetangleRect.size.height - weakSelf.animationImageSize.height, weakSelf.animationImageSize.width, weakSelf.animationImageSize.height);
    } completion:^(BOOL finished){
        if (finished == YES) {
            weakSelf.scanLine.frame = CGRectMake(weakSelf.scanRetangleRect.origin.x, weakSelf.scanRetangleRect.origin.y, weakSelf.animationImageSize.width, weakSelf.animationImageSize.height);
...
- (void)stopScanAnimation{
    self.scanLine.frame = CGRectMake(self.scanRetangleRect.origin.x, self.scanRetangleRect.origin.y, self.animationImageSize.width, self.animationImageSize.height);
...
- (UIImageView *)scanLine{
    if (_scanLine == nil) {
        _scanLine = [[UIImageView alloc] initWithFrame:CGRectMake(self.scanRetangleRect.origin.x, self.scanRetangleRect.origin.y, self.animationImageSize.width, self.animationImageSize.height)];
...
- (UIButton *)flashBtn{
    if (_flashBtn == nil) {
        ...
        [_flashBtn setImage:[UIImage imageNamed:@"KJScan.bundle/scanFlashlight"] forState:UIControlStateNormal];
