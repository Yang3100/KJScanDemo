...
#define Permissionweakself __weak typeof(self) weakSelf = self
@interface KJPermissionTool()<CLLocationManagerDelegate,CBCentralManagerDelegate>

@property(nonatomic,copy) KJPermissionToolBlock block;
...
@end
@implementation KJPermissionTool
static KJPermissionTool *_instance;
...
+ (id)allocWithZone:(struct _NSZone *)zone{
    return [KJPermissionTool shareInstance] ;
}
...
- (void)readStepCount{
    ...
    Permissionweakself;
    HKSampleQuery *query = [[HKSampleQuery alloc] initWithSampleType:stepType predicate:[KJPermissionTool predicateForSamplesToday] limit:HKObjectQueryNoLimit sortDescriptors:@[timeSortDescriptor] resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
        if(error){
...
- (void)pushSetting:(NSString*)urlStr{
    ...
    [[KJPermissionTool getCurrentVC] presentViewController:alert animated:YES completion:nil];
}
